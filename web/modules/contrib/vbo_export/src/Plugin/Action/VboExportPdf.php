...
class VboExportPdf extends VboExportBase {
  ...
  public function __construct(
    ...
    parent::__construct($configuration, $plugin_id, $plugin_definition, $stream_wrapper_manager, $temp_store_factory, $time, $file_repository, $view_data_service, $file_url_generator);
  ...
  public function buildPreConfigurationForm(array $form, array $values, FormStateInterface $form_state): array {
    ...
    $form['paper_size'] = [
      ...
      '#default_value' => $values['paper_size'] ?? 'letter',
    ];
    $form['orientation'] = [
      ...
      '#default_value' => $values['orientation'] ?? 'portrait',
    ];
  ...
  protected function generateOutput() {
    ...
    foreach ($rows as $row) {
      ...
      foreach ($header as $field_id => $label) {
        $item['fields'][] = [
          'label' => $label,
          // Tags are already stripped in the base class if strip_tags set.
          'value' => $this->configuration['strip_tags'] ? $row[$field_id] : Markup::create($row[$field_id]),
        ];
      }
    ...
    // Default to a font with full Unicode coverage so non-Latin1 glyphs (e.g.
    // Polish diacritics) render instead of being dropped by the core fonts.
    $options = new Options();
    $options->setDefaultFont('DejaVu Sans');

    $dompdf = new Dompdf($options);
    // Declare the encoding explicitly, otherwise Dompdf falls back to charset
    // auto-detection and mangles UTF-8 input.
